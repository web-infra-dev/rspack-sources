  ...
  pub fn add(&mut self, value: &'a str) {
    ...
    match &mut self.repr {
      Repr::Light(s) => {
        ...
        self.repr = Repr::Full(Arc::new(vec));
      }
      Repr::Full(data) => {
        let len = data
          .last()
  ...
  pub fn append(&mut self, value: Rope<'a>) {
    match (&mut self.repr, value.repr) {
      (Repr::Light(s), Repr::Light(other)) => {
        ...
        self.repr = Repr::Full(Arc::new(raw));
      }
...
mod tests {
  ...
  fn add() {
    ...
    assert_eq!(
      simple.repr,
      Repr::Full(Arc::new(Vec::from_iter([("abc", 0), ("def", 3)])))
    );
    ...
    assert_eq!(
      simple.repr,
      Repr::Full(Arc::new(Vec::from_iter([
        ("abc", 0),
        ("def", 3),
        ("ghi", 6),
      ])))
