...
pub fn get_map(option: GenMapOption) -> Option<SourceMap> {
    ...
    match tokens.len() {
      0 => None,
      _ => Some(SourceMap::new(Some("x".to_string()), tokens, names, sources, match sources_content.len() {
          0 => None,
...
pub fn stream_chunks_of_combined_source_map<C, S, N>(
	...
	final_source: bool,
	columns: bool
) where C: Fn(i32, i32, i32, i32, i32, i32), S: Fn(i32, String, String), N: Fn(i32, String) {
...
pub fn stream_chunks_of_source_map<C, S, N>(
    ...
    if columns {
        if final_source {
            return stream_chunks_of_source_map_final(
                source,
                source_map,
                on_chunk,
                on_source,
            ...
            return stream_chunks_of_source_map_full(
                source,
                source_map,
                on_chunk,
                on_source,
